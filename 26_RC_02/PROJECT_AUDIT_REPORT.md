...
| `arm_tools.c/h` | 工具夹爪/吸盘状态机、目标角度、安全到位判断和控制源设置。 |
...
flowchart TD
    ...
    G --> N["ArmIK_ComponentInit / clamp_init / chuck_init"]
```
...
flowchart LR
    ...
    F --> H["arm_X/Y/Z + arm_flag"]
    F --> I["tool_flag/tooluse_flag + source flag"]
```
...
- `byte 10`：`tool_flag`，0=吸盘，1=夹爪。
- `byte 11`：`tooluse_flag`，0=闭合，1=打开。
- `byte 12..35`：6 个 little-endian float，分别为底盘 3 参数 + 机械臂 x/y/z。
...
```mermaid
flowchart TD
    A["USB ARM_SET_TARGET"] --> B["Arm_task_USB"]
    C["USART arm_X/Y/Z + arm_flag"] --> D["Control_Task Arm_task"]
    B --> E["ArmIK_ComponentStep"]
    D --> E
    E --> F["Arm3R_Solve + safety"]
    F --> G["active_motor_deg"]
    G --> H["ctrl_J_USB / ctrl_J_USART"]
    H --> I["CAN_Task FDCAN3_CMD_1 J1/J2/J3"]

    J["USB/USART tool command"] --> K["trigger_chuck/clamp_action"]
    K --> L["tool state machine"]
    L --> M["target_angle / state / run_status"]
```

机械臂 IK 支持：不可达保持上一次安全目标；不安全保持上一次安全目标；无历史安全目标则保持当前不动。
...
3. **USB 源下工具状态机不运行，工具命令可能只置 `MOVING`，不会到位/超时/更新状态。**  
   `Control_Task` 只有在 `USART_Task_flag == 1` 时才运行 `chuck_state_machine_run()` 和 `clamp_state_machine_run()`；USB 工具命令在 `Data_Analysis()` 中会触发动作，但切到 USB 源时状态机不被周期调用。  
   证据：`Applications/Task/Src/Control_Task.c:106`、`Control_Task.c:109`、`Control_Task.c:110`；`Components/Algorithm/Src/Data_Analysis.c:143`、`Data_Analysis.c:145`。  
   建议：工具状态机应独立于控制源每周期运行，源只决定谁能触发动作。

4. **工具目标角度与电机输出链路未闭合，夹爪/吸盘基本不会被实际驱动。**  
   `TOOL2MOTOR` 定义为 0，导致 `CLAMP_TARGET_ANGLE` 和 `CHUCK_TARGET_ANGLE` 都是 0；`CAN_Task` 对 FDCAN3 的第 4 路电机固定输出 0，没有把 `clamp.target_angle` 或 `chuck.target_angle` 送入 PID。并且夹爪和吸盘都读取 `motor_fdcan3[3]` 作为实际角度。  
   证据：`Components/Device/Inc/arm_tools.h:14`、`arm_tools.h:15`、`arm_tools.h:16`；`Components/Device/Src/arm_tools.c:21`、`arm_tools.c:31`、`arm_tools.c:78`、`arm_tools.c:93`；`Applications/Task/Src/CAN_Task.c:77`、`CAN_Task.c:84`。  
   建议：明确工具电机 ID/通道，为夹爪/吸盘分别建立目标角度到 FDCAN 输出的闭环；如果只有一个工具电机，也需要将第 4 路 `pid_call_3()` 接上。
...
当前工具模块的控制意图已经有雏形：`trigger_*_action()` 置目标角和 `MOVING`，状态机按真实角度判断到位或超时。但实际链路明显不完整：

- 目标角度宏都为 0。
- 未将工具目标角接到 CAN 电机 PID。
- 夹爪和吸盘共用同一个 `motor_fdcan3[3]` 反馈。
- USB 源下状态机不运行。

因此工具功能应视为“框架存在，实车动作未闭环完成”。
