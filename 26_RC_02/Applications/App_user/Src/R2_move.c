 ...
 *   可随时修改目标速度。
 *
 *   R2_MODE_ROBOT_NO_YAW_VEL  机器人系速度，朝向锁定 (vw=0)
 *   R2_MODE_ROBOT_VEL         机器人系速度，朝向可变
 *   R2_MODE_WORLD_NO_YAW_VEL  世界系速度，  朝向锁定 (vw=0)
 ...
 *
 *   R2_MODE_ROBOT_NO_YAW_POS  机器人系位移，朝向锁定 (dyaw=0)
 *   R2_MODE_ROBOT_POS         机器人系位移，朝向可变
 *   R2_MODE_WORLD_NO_YAW_POS  世界系位移，  朝向锁定 (dyaw=0)
 ...
 *
 * POS 模式流程：
 *   1. 启动时将目标位移转换到机器人系（WORLD 模式做旋转）
 *   2. S 曲线在机器人系内规划
 *   3. 位置反馈取世界系位移增量，旋转到机器人系做闭环
 *   4. P 环输出直接是机器人系速度 → Mecanum_Calc
 */
                        ...
                        float robot_dy, float delta_yaw)
{
    ...
    /* 机器人系 → 世界系旋转 */
    c = cosf(ctrl->odom_yaw);
    s = sinf(ctrl->odom_yaw);

    ctrl->odom_x   += robot_dx * c - robot_dy * s;
    ctrl->odom_y   += robot_dx * s + robot_dy * c;
...
static void R2_Move_Update_Pos(R2_Move_Ctrl_t *ctrl, float now_sec)
{
    ...
    if (ctrl->pos_world_plan == 0U) {
        ...
        /*
         * yaw 可变时，需将 cmd 从 start-robot-frame
