...
/* Drive deltas below target the current 1:1 motor-output-to-wheel mechanism.
 * The previous sequence values were tuned while the old 2:1 external stage
 * was not represented in R2_CLIMB_DRIVE_COUNT_PER_MM, so their actual travel
 * is preserved here by halving the rear-drive-only moves.
 */
static const R2_ClimbMainStep_t s_main_steps[R2_CLIMB_MAIN_STEP_COUNT] = {
    ...
    {"STEP_03_DRIVE_FORWARD_60", R2_CLIMB_MAIN_STEP_DRIVE_DELTA, 0x00U, {220.0f, 220.0f, 220.0f, 220.0f}, 60.0f, R2_CLIMB_DRIVE_TIMEOUT_MS(60.0f)},
