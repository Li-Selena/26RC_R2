 ...
 */
 ...
 /* Calculate direction of gravity indicated by measurement */
	{
		float accel_norm_sq = Quat->Accel[0] * Quat->Accel[0] + Quat->Accel[1] * Quat->Accel[1] + Quat->Accel[2] * Quat->Accel[2];

		if (accel_norm_sq < 1e-6f)
		{
			/* Accel near zero — use default gravity to avoid NaN */
			Quat->QuaternionEKF.MeasuredVector[0] = 0.0f;
			Quat->QuaternionEKF.MeasuredVector[1] = 0.0f;
			Quat->QuaternionEKF.MeasuredVector[2] = 1.0f;
			Quat->AccelInvNorm = 1.0f;
		}
		else
		{
			Quat->AccelInvNorm = Fast_InverseSqrt(accel_norm_sq);
...
static void QuaternionEKF_xhat_Update(KalmanFilter_Info_TypeDef *KF){
  ...
  /* calculate the cosine of three axis orientation */
	...
	{
		float val = fabsf(KF->Data.Cache_Vector[0][i]);
			VAL_LIMIT(val, 0.0f, 1.0f);
			OrientationCosine[i] = acosf(val);
	}
