...
query YAW_TUNE
send ARM_SET_TARGET 0 1 450 0
send TOOL_SET_MODE 2 1 450 0
...
USART 遥控帧 `byte10..12` 当前保留；新机械臂工具目标通过 USB `ARM_SET_TARGET/TOOL_SET_MODE` 设置。上台阶字段位于 `byte13..15`：`climb_enable / climb_step / climb_auto`。步进和自动都按 `0->1` 上升沿触发；自动启动后仍要持续发送 `--source-usart --climb-enable` 保活帧，否则固件 `300ms` USART 看门狗会停止 USART 侧控制器。
