...
static HAL_StatusTypeDef RobStride_FDCAN_AddTxMessage(const CAN_TxHeaderTypeDef *legacy_header, uint8_t *txdata)
{
    FDCAN_TxHeaderTypeDef tx_header = {0};
    FDCAN_ProtocolStatusTypeDef psr;
    HAL_StatusTypeDef status;

    tx_header.Identifier = (legacy_header->IDE == CAN_ID_EXT) ? legacy_header->ExtId : legacy_header->StdId;
    tx_header.IdType = (legacy_header->IDE == CAN_ID_EXT) ? FDCAN_EXTENDED_ID : FDCAN_STANDARD_ID;
    tx_header.TxFrameType = FDCAN_DATA_FRAME;
    tx_header.DataLength = RobStride_FDCAN_Dlc(legacy_header->DLC);
    ...
    status = HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan3, &tx_header, txdata);
    if (status == HAL_OK)
    {
        g_can_diag.tx_ok++;
    }
    ...
    g_can_diag.last_hal_err = HAL_FDCAN_GetError(&hfdcan3);
    if (HAL_FDCAN_GetProtocolStatus(&hfdcan3, &psr) == HAL_OK)
    {
...
#define HAL_CAN_AddTxMessage(unused_hcan, legacy_header, txdata, unused_mailbox) RobStride_FDCAN_AddTxMessage((legacy_header), (txdata))
/*******************************************************************************
