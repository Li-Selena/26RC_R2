...
static uint8_t dm8006_send_mode_command(FDCAN_HandleTypeDef *hcan, uint16_t motor_id,
                                        uint16_t mode, uint8_t command)
{
    uint8_t cmd[8] = {0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, 0xFFU, command};
    return canx_send_data(hcan, (uint16_t)(motor_id | mode), cmd, 8U);
}

uint8_t dm8006_enter_motor_mode(FDCAN_HandleTypeDef *hcan, uint16_t motor_id)
{
    return dm8006_send_mode_command(hcan, motor_id, MIT_MODE, 0xFCU);
}

uint8_t dm8006_exit_motor_mode(FDCAN_HandleTypeDef *hcan, uint16_t motor_id)
{
    return dm8006_send_mode_command(hcan, motor_id, MIT_MODE, 0xFDU);
}

uint8_t dm8006_set_zero_position(FDCAN_HandleTypeDef *hcan, uint16_t motor_id)
{
    return dm8006_send_mode_command(hcan, motor_id, MIT_MODE, 0xFEU);
}

uint8_t dm8006_enter_pos_speed_mode(FDCAN_HandleTypeDef *hcan, uint16_t motor_id)
{
    return dm8006_send_mode_command(hcan, motor_id, POS_MODE, 0xFCU);
}

uint8_t dm8006_exit_pos_speed_mode(FDCAN_HandleTypeDef *hcan, uint16_t motor_id)
{
    return dm8006_send_mode_command(hcan, motor_id, POS_MODE, 0xFDU);
}

uint8_t dm8006_set_zero_position_pos_speed(FDCAN_HandleTypeDef *hcan, uint16_t motor_id)
{
    return dm8006_send_mode_command(hcan, motor_id, POS_MODE, 0xFEU);
}
                                      ...
                                      float pos, float vel)
{
    ...
    memcpy(&cmd[0], &pos, sizeof(pos));
    memcpy(&cmd[4], &vel, sizeof(vel));

    return canx_send_data(hcan, (uint16_t)(motor_id | POS_MODE), cmd, 8U);
}
