...
- `tool: 0=S1, 1=S2, 2=gripper`；`state: 0=stow, 1=use`。
...
- `approach_yaw_rad` 直接作为 J1 目标，范围由 `ROBOTARM_KIN_J1_MIN/MAX_RAD` 限制。
...
3. 发送 `ARM_SET_TARGET`。
4. 查询 `ARM_GET_STATUS`，检查 IK 状态、目标角和工具世界坐标。
5. 停止时发 `ARM_STOP`。
...
3. 发 `TOOL_SET_MODE tool state target_z_mm approach_yaw_rad`，其中 `tool: 0=S1, 1=S2, 2=gripper`，`state: 0=stow, 1=use`。
...
| 小驱动轮驱动块 | `120mm / 360mm / 1530mm`，完整上/下台阶流程为前后四个小驱动轮一起驱动 |
...
| 小驱动轮后退块 | `1600mm / 260mm / 500mm`，完整上/下台阶流程为前后四个小驱动轮一起驱动 |
| 小驱动轮超时估算 | `80mm/s + 1500ms` |
...
5. 当前完整顺序已压缩为 `11` 个固件状态：连续立杆 `10mm` 动作合并为 `30mm` 目标，连续小驱动轮前进合并为 `1530mm`，重复后腿下压和底盘前进也各合并为一段
...
| `10..12` | 保留；新机械臂工具目标通过 USB `ARM_SET_TARGET/TOOL_SET_MODE` 设置 |
...
### 6.3 `ARM_GET_STATUS` packet `0x26`, `LEN=64`

`TOOL_GET_STATUS (0x36)` uses the same 64-byte layout in the current firmware.
...
| 4 | u8 | `status_flags` |
| 5 | u8 | `error_flags` |
| 6 | u8 | `ik_status`, `0=OK, 1=NULL, 2=BAD_PARAM, 3=HEIGHT_UNREACHABLE, 4=JOINT_LIMIT` |
| 7 | u8 | `ik_reason`, `0=NONE, 1=BAD_TOOL, 2=BAD_STATE, 3=BAD_FLOAT, 4=HEIGHT_OUTSIDE_LINK, 5=J1_LIMIT, 6=J2_LIMIT, 7=J3_LIMIT` |
| 8 | u32 | `last_command_ms` |
| 12 | u32 | `last_update_ms` |
| 16 | u32 | `output_apply_count` |
| 20 | u8 | `tool`, `0=S1, 1=S2, 2=gripper` |
| 21 | u8 | `tool_state`, `0=stow, 1=use` |
| 24 | f32 | `target_z_mm` |
...
`error_flags`: bit0=IK_NOT_OK, bit1=JOINT_LIMIT, bit2=HEIGHT_UNREACHABLE, bit3=BAD_PARAM.

### 6.4 `TOOL_GET_STATUS` packet `0x36`, `LEN=64`
...
| 233 | u8 | `arm.tool_state`，`0=stow, 1=use` |
| 236..237 | u8 | 保留 |
